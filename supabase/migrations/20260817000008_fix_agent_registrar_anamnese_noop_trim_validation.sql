-- Fecha 3 achados da revisao final de agent_registrar_anamnese, todos sobre
-- a mesma funcao (revisao encontrou depois que 20260817000007 ja tinha
-- corrigido o coalesce vs "?" -- essa correcao anterior e mantida aqui):
--
-- (a) Guarda de linha fantasma [Important]: chamar com p_campos = '{}' e
--     p_observacao = null num paciente SEM Anamnese ainda rodava o upsert
--     mesmo assim, criando uma linha permanente com os 11 campos null e sem
--     followup -- vira silenciosamente "Nenhuma anamnese registrada ainda."
--     em grade de onze "-". Espelha o guard ja existente no caminho web
--     (web/lib/actions/anamnese.js, commit 99621d0).
-- (b) Trim / string vazia vira null [Important]: o caminho web faz
--     valor?.trim() || null (commit 99621d0, corrigido porque edicoes
--     so-com-espaco geravam eventos de followup espurios). A RPC usava o
--     valor cru de p_campos->>'x' em todo lugar -- agora usa
--     nullif(trim(p_campos->>'x'), '') tanto na deteccao de diff quanto na
--     persistencia, nos 11 campos.
-- (c) Guarda de p_campos malformado [Minor]: se p_campos for array/escalar
--     (nao objeto), jsonb_object_keys(p_campos) levantava um erro cru do
--     Postgres em vez de um codigo de negocio. Agora valida o tipo antes de
--     qualquer outra coisa e levanta CAMPOS_INVALIDOS.
