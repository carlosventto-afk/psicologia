...
- **3 RPCs novas** (`service_role`-only, mesmo padrão das 16 existentes):
  ...
  - `agent_criar_conta_bancaria(p_whatsapp_number, p_nome, p_banco, p_agencia default null, p_numero default null, p_tipo default null)` — cria conta bancária pro profissional (inicia pagamentos da plataforma); **não existe campo `chave_pix`**; gera `codigo` sequencial (`C001`, `C002`, ...) derivado do maior sufixo numérico já usado entre códigos no formato `C0xx` do owner (não conta linhas — corrigido na revisão final pra não colidir depois de uma exclusão, migration `20260904000006`)
