...
| 7 | Emitir Nota Fiscal (NFS-e) direto pelo sistema — microserviço Python novo (`nfse-service/`, vendoriza o kit `NotaFiscal/nfse-nacional-kit`) + telas `/configuracoes/nfse` (dados fiscais, certificado A1, troca homologação→produção irreversível) e `/notas-fiscais` (emissão e cancelamento), com envio automático por e-mail via Resend | 2026-08-14 | Self-service pra qualquer profissional. `NotaFiscal` fonte de dados é `PagamentoSessao` (valor realmente pago), mesmo padrão do item 8. Sem gerador de PDF próprio nesta v1 (fallback é "tentar de novo depois"). Verificado end-to-end em homologação real (chamada mTLS de verdade contra a SEFIN, rejeitada como esperado por certificado sintético não cadastrado) — **falta só a primeira emissão com certificado A1 real do profissional + infra de produção** (deploy do serviço `psiagente-nfse` no EasyPanel, env vars, conta Resend — ver notas de infra no plano). Código pushado pra produção |
