...
- Schema confirmado de `agent_sessions` hoje (**correção pós-planejamento**: a coluna `consultorio_ativo_id` citada na primeira versão deste parágrafo já não existe — dropada por `20260827000002_agent_rpc_remove_consultorio_scope.sql`, junto com o escopo por consultório do agente inteiro): `whatsapp_number text primary key`, `usuario_id bigint references "Usuarios"(id) on delete cascade`, `updated_at timestamptz not null default now()`. RLS já é deny-all pra `anon`/`authenticated` (`20260727000004_lockdown_agent_tables.sql`) — as colunas novas da Task 1 herdam essa proteção automaticamente, sem precisar mexer em RLS/grants.
- **Achados da revisão final de branch (2026-09-04)**, corrigidos numa única rodada de fix após todas as 9 tasks: `reenviar_link`/`revalidar` não tinham limite anti-abuso próprio (reabria o vetor de spam de e-mail que o limite existe pra fechar — corrigido pra compartilhar o mesmo contador de `criar_conta`); o webhook de retomada em `continuarFluxoWhatsapp` era `await`ado sem timeout, podendo travar o redirect de login de **qualquer** usuário (não só WhatsApp) se o webhook (ainda inexistente) nunca responder — corrigido com `AbortSignal.timeout`; os desvios `WHATSAPP_JA_CADASTRADO`/`EMAIL_JA_CADASTRADO` reportavam sucesso mesmo quando o reenvio do link mágico falhava — corrigido pra também retornar `ERRO_ENVIAR_LINK` nesse caso; `agent_sessions.onboarding_etapa` não tinha `check` constraint (um typo do futuro workflow n8n travaria a isenção de plano pra sempre) — adicionada; `codigo` de `agent_criar_conta_bancaria` colidia após excluir uma conta (`count(*) + 1` não é estável) — trocado pra derivar do maior sufixo numérico existente; e-mail não era normalizado (`trim`/`lowercase`), podendo causar falso `EMAIL_JA_CADASTRADO` sem link enviado — corrigido. Ver o relatório final de revisão (`.superpowers/sdd/2026-09-04-inicio-operacao-whatsapp-backend/`, já removido após o merge) e o commit de fix pra detalhes completos.
