...
**Interfaces:**
- Consumes: `_agent_resolve_consultorio(text, bigint)` (já existe).
...
begin
  ...
  insert into "Paciente" (nome, telefone, email, valor_sessao, consultorio)
  values (
    btrim(p_nome),
  ...
  insert into "ResponsavelFinanceiro" (nome, paciente_vinculado)
  values (btrim(p_nome), v_paciente_id)
  returning id into v_responsavel_id;

  insert into "PacienteResponsavelFinanceiro" (paciente, responsavel)
  values (v_paciente_id, v_responsavel_id);
